...
export default function StorefrontSetupPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <BackLink href="/storefront" />
      <PageHeader title="Set up your storefront" description="Your store's presentation, address, payouts, going live, discounts and custom domains — all in one place." />
...
function PayoutCard({
  ...
  // Stripe payout is paused while we focus on Flutterwave. Keep the mutation
  // around (commented) so re-enabling later is a one-line change.
  ...
  return (
    <Card title="Payouts">
      ...
      <div className="flex flex-col gap-4">
        {/* Stripe payout paused while we focus on Flutterwave. Uncomment this
            block (and the connectStripe mutation above) to bring it back.
...
// The viewer's IANA time zone (e.g. "America/New_York"), shown next to the
// date fields so it's clear the times they type are in *their* local zone.
const LOCAL_TZ =
  typeof Intl !== "undefined" ? Intl.DateTimeFormat().resolvedOptions().timeZone : "local time";

// Format a stored UTC timestamp back into the viewer's local date+time.
...
function DiscountsCard({ onError, clearError }: { onError: (e: unknown) => void; clearError: () => void }) {
  ...
  const create = useMutation({
    mutationFn: () =>
      api.post("/storefront/discounts", {
        ...
        // A datetime-local string has no zone; new Date() reads it in the
        // viewer's tz, and toISOString() sends the absolute instant (UTC).
        startsAt: startsAt ? new Date(startsAt).toISOString() : undefined,
        endsAt: endsAt ? new Date(endsAt).toISOString() : undefined,
      }),
    onSuccess: () => { setCode(""); setValue(""); setStartsAt(""); setEndsAt(""); void invalidate(); },
    onError,
  });
  const setActive = useMutation({
    mutationFn: ({ id, active }: { id: string; active: boolean }) =>
      api.patch(`/storefront/discounts/${id}/active`, { active }),
    onSuccess: invalidate,
    onError,
  });
  const remove = useMutation({
  ...
  return (
    <Card title="Discount codes">
      ...
      {/* Optional schedule. Times are in the viewer's own zone. */}
      ...
      <p className="mt-1 text-[11px] text-text-subtle">Times are in your time zone ({LOCAL_TZ}).</p>

      <div className="mt-3">
        <Button variant="outline" loading={create.isPending} onClick={() => { clearError(); create.mutate(); }}>Add code</Button>
      </div>

      <div className="mt-4 flex flex-col gap-2">
        {(codes.data ?? []).map((c) => {
          const starts = formatWindow(c.starts_at);
          const ends = formatWindow(c.ends_at);
          const busy = setActive.isPending || remove.isPending;
          return (
            <div key={c.id} className="rounded-md border border-line px-3 py-2 text-body-sm">
              ...
              {starts || ends ? (
                <div className="mt-1 text-[11px] text-text-subtle">
                  {starts ? `From ${starts}` : "No start"}{" · "}{ends ? `until ${ends}` : "no end"}
                </div>
              ) : null}
              <div className="mt-2 flex gap-4">
                <button type="button" disabled={busy}
                  onClick={() => { clearError(); setActive.mutate({ id: c.id, active: !c.active }); }}
                  className="text-[12px] font-medium text-amber hover:underline disabled:opacity-50">
