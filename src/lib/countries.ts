 ...
 *
 * Used by any form that captures a country of origin / shipping
 * country. The two-letter `code` is what we store + send to the
 * backend (carriers + customs expect ISO 3166-1 alpha-2); the `name`
 * is the human label shown in dropdowns. Sorted alphabetically by
 * name so users can scan quickly.
 *
 * `dialCode` is the E.164 international calling code prefix
 * (formatted "+XXX", no spaces). Used by the KYC contact step to
 * bundle a country picker beside the phone number input so the
 * vendor doesn't have to remember to type the country code prefix
 * by hand. Where a country shares the +1 NANP code with the US and
 * Canada we still record "+1" — that's the ITU primary assignment
 * and KYC doesn't need the trailing area code portion. Crown
 * Dependencies (Guernsey, Jersey, Isle of Man) share the UK's "+44".
 * Ascension is "+247"; Saint Helena and Tristan da Cunha share
 * "+290"; Curaçao, Sint Maarten, and the BES Islands share "+599".
 *
 * Source: ISO 3166-1 alpha-2 + ITU-T E.164 (current as of 2025).
 * Maintained by hand because the list rarely changes and pulling a
 * library is overkill. If a new country is recognised
 * internationally, add it here and re-export.
 *
 * Lookup helpers (`getCountryName`, `isValidCountryCode`,
 * `getDialCode`) live at the bottom so callers don't have to scan
...
export interface Country {
  ...
  code: string;
  /** Common English name suitable for UI display. */
  name: string;
  /**
   * E.164 international calling code prefix, e.g. "+234" for Nigeria
   * or "+1" for the United States. Always includes the leading "+",
   * no internal spaces. Used by the KYC contact step.
   */
  dialCode: string;
}
...
export const COUNTRIES: ReadonlyArray<Country> = [
  ...
  { code: "SX", name: "Sint Maarten (Dutch part)", dialCode: "+599" },
...
/**
 * Build a Map for O(1) lookups. Lazy-initialised on first access so
 * tree-shakers can drop it for callers that only need COUNTRIES.
 */
let codeIndex: Map<string, Country> | null = null;
function ensureIndex(): Map<string, Country> {
...
/** Returns the country name for a code, or null when unrecognised. */
export function getCountryName(code: string): string | null {
  if (!code) return null;
  return ensureIndex().get(code.toUpperCase())?.name ?? null;
}

/** True when the given string is a recognised ISO 3166-1 alpha-2 code. */
export function isValidCountryCode(code: string): boolean {
  if (!code) return false;
  return ensureIndex().has(code.toUpperCase());
}

/**
 * Returns the E.164 dial-code prefix (e.g. "+234") for an ISO 3166-1
 * alpha-2 country code, or null when the code is unrecognised. Used
 * by the KYC contact step to compose the canonical phone number from
 * the chosen country plus the locally-typed digits.
 */
export function getDialCode(iso2: string): string | null {
  if (!iso2) return null;
  return ensureIndex().get(iso2.toUpperCase())?.dialCode ?? null;
}
